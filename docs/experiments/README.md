...
| Online compensation v1 | 原公开 24 × 4 回归，另做 80 次动态误差对照 | 24-case 在线切向中位数 1.359 mm；动态误差在线阶段 40/42 | 保留在线补偿和两条换向姿态失败；未替换学习实验基线 |
| Rotation gain preset | 相同 80 次动态误差对照；所有方法 Krot × 2、Drot × sqrt(2) | 在线阶段 42/42；自身全程平均切向 RMSE 2.559 → 2.575 mm；积分仍有两条失败 | 可选常量预设，默认不变；先检查原 24-case 几何网格 |
...
| Reversal recovery transfer v1 | 3 yaw × 2 负载 × 3 误差 × 2 噪声种子 × 2 方法；新增 64 条、引用原 8 条 | 34/36，整体 `FAIL`；降载 18/18，高负载 16/18；−15° 无额外误差的两条加速代价超限，绝对工程检查均通过 | [全部配对与保留的反例](../reversal_recovery_transfer.md)；未调参，不扩大采用范围，不改写原测量回归的 7/8 |
| Stationary recovery pilot v1 | −15°、无额外误差；两种负载 × 两个噪声种子；新增 4 条、引用 8 条 | 3/4，整体 `FAIL`；高负载 seed 29 仍超过位置与速度代价门槛 | [仅静止时回退](../stationary_recovery_pilot.md)；局部改善，不推广 |
| Reversible recovery v1 | 入口系数限幅；4 条小试通过后补跑 32 条，扩展评价 108 条，其中引用 76 条 | 小试 4/4、扩展 32/36，整体 `FAIL`；修复两条高负载失败，却新增 +15° 组合误差下四条降载速度失败 | [高负载恢复与降载追赶的取舍](../reversible_recovery.md)；保留所有轨迹和原门槛，不推广、不改默认 |
| Measured-load C++ replay v1 | 上述 27 条轨迹＋1 条重复演示，168,000 拍 | Python/C++ 状态和命令逐拍一致 | [控制链与边界](../cpp_core.md)；重复演示不算独立场景，不是真机实时性证据 |

## 版本锚点与证据

上表分别注明各轮实验的数据身份和适用边界。旧精简轨迹缺少的测量状态
...
在线补偿先在原 24-case、4.5 秒任务中复现旧三种方法的全部 72 行指标，再增加在线方法。
另做的 80 次误差实验使用固定 case 16 几何配置，在线阶段检查为 40/42；两个反向加速段
相对无补偿基线的姿态 RMSE 增量超过 0.1°。

后续只改变常量姿态阻抗，四种方法统一旋转刚度乘 2、阻尼乘 sqrt(2)，重新运行同一协议。
在线通过 42/42，积分仍有两条阶段失败；全体方法的阶段失败由 12 条降为 2 条，没有新增
失败。在线切向 RMSE 均值增加 0.016 mm，速度误差 RMS 均值增加 3.71%，因此暂不更改默认值。
两份 80 次归档各保留全部紧凑轨迹和四份完整轨迹；两种增益各另完成 24,000 步 C++ 回放。

[在线补偿公式与原实验](../online_compensation.md)和[姿态增益对照](../rotation_gain_comparison.md)
分别说明更新律与调参过程。数值以[原 80 次对照](../../results/franka_online_compensation_errors/)
及[新 80 次对照](../../results/franka_rotation_gain_comparison/)为准，旧失败归档未改动。

随后完成[原 24-case 双增益验证](../rotation_gain_public24.md)：12 个表面方向／接触时间／
模型已知质量组合，各两个噪声种子。原96条既有指标最大差为0；192次本轮工程检查全部通过。
在线姿态误差在24个case均降低，切向位置和速度误差在24个case均增加，故没有推广为全局默认。
独立审计重算全部轨迹指标及配对统计，预选12份完整轨迹完成27,000步Python/C++核验。
