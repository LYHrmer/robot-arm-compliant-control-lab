...
这页记录训练前的接口、数据和经典对照。实际小规模训练另见
[行为克隆与残差 PPO](surface_learning_pilot.md)，两类证据分开保存。
...
准备阶段没有训练新策略，也不增加 ROS、Franka 硬件适配或在线学习。
后续首轮 BC / PPO 的[训练方法](surface_learning_pilot.md)与
[所有种子结果](../results/franka_surface_learning_pilot/)单独记录。
解析强基线已经通过准备域的全部跟踪目标，学习算法仍须与它比较。
