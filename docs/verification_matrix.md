...
| 停顿回退局部 4/4；跨方向与误差回归 34/36，整体 `FAIL` | [回退控制器](../tools/reversal_recovery/controller.py)、[跨方向生成与审计](../tools/reversal_recovery_transfer/study.py) | [协议与门槛](../tests/test_reversal_recovery.py)、[跨方向身份与篡改拒绝](../tests/test_reversal_recovery_transfer.py) | [原 4 对](../results/franka_reversal_recovery/comparison.json)、[36 对评价](../results/franka_reversal_recovery_transfer/comparison.json)；[降载 18/18、高负载 16/18](reversal_recovery_transfer.md)，属公开开发，不是新 holdout，默认不变 |
| 只在目标静止时回退的单因素 pilot 通过 3/4，高负载 seed 29 仍超原位置与速度门槛 | [静止回退控制器](../tools/stationary_recovery/controller.py)、[pilot 生成与审计](../tools/stationary_recovery/study.py) | [系数规则](../tests/test_stationary_recovery_controller.py)、[配对身份与 5.5 s 前缀](../tests/test_stationary_recovery_study.py) | [12 条评价](../results/franka_stationary_recovery_pilot/comparison.json)；[静止回退试验](stationary_recovery_pilot.md)整体 `FAIL`，默认不变，原 4/4 与 34/36 保留 |
| 保持期可逆恢复受入口系数约束；小试 4/4，但扩展 32/36，修复两个旧反例并新增四个速度失败 | [候选规则](../tools/reversible_recovery/controller.py)、[两级协议](../tools/reversible_recovery/study.py)、[独立辅助状态复算](../tools/reversible_recovery/audit.py) | [长序列不变量](../tests/test_reversible_recovery_invariants.py)、[协议拒绝路径](../tests/test_reversible_recovery_study.py)、[真实归档与速度复算](../tests/test_published_reversible_recovery.py) | [16 条小试评价](../results/franka_reversible_recovery_pilot/comparison.json)、[108 条扩展评价](../results/franka_reversible_recovery_transfer/comparison.json)；[高负载改善、降载追赶变差](reversible_recovery.md)，不推广，无候选 C++ 或真机结论 |
