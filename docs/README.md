...
2. 先看[系统架构](architecture.md)的五节点概览，再按需展开实时控制或实验冻结流程。
3. 打开[v0.5 first-reveal 摘要](../results/franka_safety_blind/summary.md)，确认预注册结论是
   `FAIL`，再读[为什么暂不部署](residual_rl_decision.md)。
4. 需要核实某项说法时，到[验证矩阵](verification_matrix.md)找源码、测试和实验产物。
...
| 实验 | 说明与命令 | 已保存产物 |
|---|---|---|
| 2-DOF 与 Franka 标称场景 | [首页快速运行](../README.md#快速运行) | [`results/`](../results/)、[`results/franka/`](../results/franka/) |
...
在 [post-reveal summary](../results/franka_safety_postreveal/summary.md)，不改写 first-reveal
目录中的任何文件。
...
- **教程**回答“为什么这样算”，允许按章节逐步推导。
- **算法参考**回答“代码具体怎样实现”，适合结合源码查参数和状态机。
- **实验记录**回答“何时冻结、用了哪些数据、结果是什么”，不会替代算法说明。
