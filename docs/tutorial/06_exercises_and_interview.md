...
4. 已有小规模实验，可复现：先检查[换向恢复候选](../reversal_recovery.md)的四对比较，再看[扩大回归](../reversal_recovery_transfer.md)的 34/36。解释“高负载下短暂停顿后恢复”的代价为什么在 −15° 超限；下一轮若改算法，先诊断这两条反例，不改默认值、不扩展 RL。
