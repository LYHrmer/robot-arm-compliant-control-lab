...
针对这一残差完成[12 次单因素诊断](docs/combined_residual_diagnosis.md)：原组合工况后段
有 95.6%–98.0% 的采样触发切向补偿幅值限制，主要表现为沿轨迹滞后。保持低摩擦时
误差减小，但移除测量偏置反而增大切向误差；这是输入反事实结果，不是算法提升。
控制器及 6 N 上限保持不变，逐周期诊断轨迹和配对差值均可独立复核。
