"""Independent auxiliary-state replay for the stationary-only hold-cap pilot.

Copied from tools.reversal_recovery.study.replay_compensation so the published
auditor remains frozen. The only changed control equation is the release gate:
stationary_hold_cap requires target tangential speed <= 1e-12 m/s. The old
methods retain their original equations. This module never calls a controller
or integrates dynamics; its caller must also check full schema, case schedules,
metadata, packet source provenance and archive identity.
"""
...
METHODS = {"adaptive6_8", "hold_cap_tracking", "stationary_hold_cap"}
