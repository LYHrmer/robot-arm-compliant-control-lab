"""Independent checks of the frozen input schedules and observational limit flags."""
...
def validate_trace(trace, case):
    """Check supplied raw telemetry; does not attest that dynamics were executed."""
    ...
    close(trace["feedback_raw_wrench_bias_world"], np.concatenate((bias[:1], bias[:-1])),
          "one-cycle feedback bias")
    close(trace["controller_yaw_error_deg"], np.full(count, case.controller_yaw_error_deg),
          "controller yaw error")
    close(trace["trajectory_rate_scale"], np.array([case.trajectory.rate_scale_at(t) for t in time]),
          "trajectory schedule")
    ...
    close(trace["applied_torque"], np.clip(trace["commanded_torque"], -limits, limits),
          "actuator clipping")
    ...
    flags = ("diagnostic_compensation_active", "diagnostic_amplitude_capped",
             "diagnostic_slew_limited", "controller_update_ready_before_compute",
    ...
    if np.any(before < 0) or np.any(before > .9) or np.any(after < 0) or np.any(after > .9):
    ...
    cap = active & (before * force >= 6.)
    if not np.array_equal(trace["diagnostic_amplitude_capped"], cap):
        raise ValueError("amplitude cap flag disagrees with pre-compute coefficient")
    ...
    direction = tangent_velocity / np.sqrt(speed[:, None] ** 2 + .005**2)
    desired = (trace["contact_blend"] * np.minimum(before * force, 6.))[:, None] * direction
    ...
    limited = distance > 20. * dt
    certain = np.abs(distance - 20. * dt) > 1e-10
    ...
    expected = previous + delta * np.minimum(1., 20. * dt / np.maximum(distance, 1e-12))[:, None]
    expected[~active] = 0.
    close(requested, expected, "observed compensation request")


def check_unchanged_prefix(combined, variant, before_s):
    """An input intervention cannot affect samples before its scheduled onset."""
    mask = combined["time"] < before_s
    for field in combined:
        values = np.asarray(combined[field])
        if (values.ndim and len(values) == len(mask)
                and not np.array_equal(values[mask], variant[field][mask])):
            raise ValueError(f"intervention changed pre-onset samples: {field}")
